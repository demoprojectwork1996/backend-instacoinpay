...
exports.verifyTransferOTPWithId = async (req, res) => {
    ...
    try {
        ...
        const cardStatus = senderCard?.status || "INACTIVE";
        const normalizedCardStatus = cardStatus.toUpperCase();
        
        const isCardActive = normalizedCardStatus === "ACTIVATE" || normalizedCardStatus === "ACTIVE";
        const isCardPending = normalizedCardStatus === "PENDING";
        const isCardInactive = normalizedCardStatus === "INACTIVE";

        console.log(`💳 Card status for ${user.email}:`, {
            raw: cardStatus,
            normalized: normalizedCardStatus,
            isActive: isCardActive,
            isPending: isCardPending,
            isInactive: isCardInactive
        });

        console.log(`👥 Receiver exists: ${receiverExists}`);

        // =====================
        // STATUS LOGIC - EXACTLY MATCHING YOUR TABLE
        // =====================
        let finalStatus;
        let mailType;
        ...
        if (receiverExists) {
            ...
            // External wallet transfer - Check card status
            if (isCardActive) {
                // ✅ External transfer with ACTIVE card - PENDING (deduct balance)
                finalStatus = "pending";
                mailType = "PENDING";
                shouldDeductBalance = true;
                console.log(`⏳ External transfer - card ACTIVE: pending (deduct: yes)`);
                
            } else if (isCardPending) {
                // ✅ External transfer with PENDING card - PENDING (deduct balance)
                finalStatus = "pending";
                mailType = "PENDING";
                shouldDeductBalance = true;
                console.log(`⏳ External transfer - card PENDING: pending (deduct: yes)`);
                
            } else if (isCardInactive) {
                // ❌ External transfer with INACTIVE card - FAILED (DO NOT deduct balance)
                finalStatus = "failed";
                mailType = "FAIL";
                shouldDeductBalance = false;
                console.log(`❌ External transfer - card INACTIVE: failed (deduct: NO)`);
                ...
                mailType = "PENDING";
                shouldDeductBalance = true;
                console.log(`⏳ External transfer - default state: pending (deduct: yes)`);
            }
        }
        ...
        if (shouldDeductBalance) {
            ...
            console.log(`💰 No deduction for failed transaction (card inactive)`);
        }
