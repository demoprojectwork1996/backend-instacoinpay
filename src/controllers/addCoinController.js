...
const sendTransactionMail = require("../utils/sendZeptoTemplateMail"); // Changed import
...
const sendBalanceUpdateEmail = async (user, asset, actionType, amount, newBalance) => {
  try {
    ...
    if (actionType === "credited") {
      templateKey = process.env.TPL_ADMIN_BALANCE_UPDATE; // Credit template
    } else if (actionType === "debited") {
      templateKey = process.env.TPL_ADMIN_BALANCE_DEBIT; // Debit template
    }
    ...
    const emailVariables = {
      ...
      // Add additional variables specific to debit/credit if needed
    ...
    // Use sendTransactionMail (the function you have)
