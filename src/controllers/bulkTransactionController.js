...
// ─── generates a realistic-looking BEP-20 / EVM transaction hash ─────────────
  ...
  try {
    ...
    for (const user of selectedUsers) {
      try {
        ...
        // ✅ Generate a unique tx hash per user — replaces "Admin Wallet"
        const txHash = generateTxHash();

        const transfer = await Transfer.create({
          fromUser:    user._id,
          toUser:      user._id,
          fromAddress: type === "CREDIT" ? txHash        : userWalletAddress,  // ✅ random hash, not "Admin Wallet"
          ...
          notes:       JSON.stringify({
            type:        type === "CREDIT" ? "BULK_CREDIT" : "BULK_DEBIT",
            displayTxId: txHash,                                                // ✅ stored for frontend display
          }),
          createdAt:   new Date(),
        ...
        await sendZeptoTemplateMail({
          ...
          mergeInfo: {
            ...
            amount:      numericAmount,
            asset:       coin,
            txId:        txHash,           // ✅ email now shows the realistic tx hash too
            status:      "COMPLETED",
